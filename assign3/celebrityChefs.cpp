...
const DishModifier
		...
		= { DishModifier("triple deep-fried",BEFORE_MODLOC),
		    ...
		    DishModifier("with a shot of Trump-endorsed bleach",
				 AFTER_MODLOC
				)
		  };
  ...
  void		serve		(Chef&		chef,
		  ...
		  //  MAKE THIS THREAD-SAFE, PLEASE
			...
			while (dishPtr_ != NULL) {
				...
				std::cout
				<< chef
				<< ": \"WOULD YOU CLEAR THAT EFF-ING DISH OFF THE TABLE!\"\n";
				pthread_mutex_unlock(&tlock);
			...
			)
{
  ...
  //  PERHAPS A LOOP HERE
	for (int i = 0; i < NUM_DISHES_TO_DO; i++) {
		Dish* prep = (chefPtr)->prepare();
		table.serve(*chefPtr, prep);
	}
	delete(chefPtr);
			...
			)
{
  ...
  //  PERHAPS A LOOP HERE
	for (int i = 0; i < NUM_DISHES_TO_DO; i++) {
		Dish* eatThis = table.eatFrom(*gourmandPtr);
		(gourmandPtr)->consume(eatThis);
	}
	delete(gourmandPtr);
...
//  PURPOSE:  To harass CSC 374 students into learning thread programming
